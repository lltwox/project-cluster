...
Stat.prototype.initEvents = function() {
  ...
  this.master.on('worker.startfailed', this.handleWorkerStartFailed.bind(this));
...
/**
 * Handler for event about worker failure to start
 *
 * @param  {cluster.Worker} worker
 * @private
 */
Stat.prototype.handleWorkerStartFailed = function(worker) {
  if (!this.stats[worker.uid]) return;
  this.stats[worker.uid].restarted++;
};
