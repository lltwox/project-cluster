...
MasterProcess.prototype.stop = function() {
  ...
  this.removeAllWorkers(function() {
    this.logger.info('Cluster stopped');
    this.emit('stop');
  }.bind(this));
};
 ...
 */
MasterProcess.prototype.addWorker = function(params, callback) {
  if (this.stopInProgress) return;

  var worker;
  try {
    worker = this.fork(params);
  } catch (err) {
  ...
  var onExit = function() {
    clearTimeout(this.startTimers[worker.id]);
    this.emit('worker.startfailed', worker);
...
MasterProcess.prototype.restartWorker = function(id, callback) {
  ...
  this.addWorker({
    type: worker.type, uid: worker.uid
  }, function(err, newWorkerId) {
...
MasterProcess.prototype.restartAllWorkers = function(type, callback) {
  ...
  async.mapSeries(currentWorkers, function(id, callback) {
    var worker = this.getWorker(id);
    restartedUids.push(worker.uid);
    if (!worker) return callback(null, 'Unknown worker: ' + id);

    this.addWorker({type: worker.type, uid: worker.uid}, function(err) {
      if (err) return callback(null, err.message);

      this.removeWorker(id, function(err) {
        if (err) return callback(null, err.message);
        callback();
      });
    }.bind(this));
  ...
  function(err, results) {
    if (err) return callback && callback(err);
    callback(null, _.zipObject(restartedUids, results));
  }.bind(this));
};
...
MasterProcess.prototype.initWorkerEvents = function(worker) {
  ...
  var scheduleRestart = function() {
    ...
    this.restartTimers[worker.uid] = setTimeout(function() {
      this.addWorker({type: worker.type, uid: worker.uid});
...
MasterProcess.prototype.forkWorkerProcesses = function(callback) {
  ...
  async.eachSeries(apps, function(type, callback) {
    ...
    async.times(
      workersNumber,
      function(n, callback) {
        this.addWorker({type: type}, function(err) {
 ...
 * @private
 */
MasterProcess.prototype.fork = function(params) {
  var type = params.type;
  if (!type) throw new Error('Missing type');

  this.workers[type] = this.workers[type] || {};
  this.workersUids[type] = this.workersUids[type] || {};
  var uid = params.uid || this.generateWorkerUid(type);

  var env = _.clone(process.env);
  if (params.env) {
    this.workersEnv[uid] = params.env;
    _.extend(env, params.env);
  } else if (this.workersEnv[uid]) {
    _.extend(env, this.workersEnv[uid]);
  }
  env.WORKER_TYPE = type;

  var worker;
  if (this.config.spawn == MasterProcess.SPAWN_CLUSTER) {
    worker = new WorkerCluster(cluster.fork(env));
  } else {
